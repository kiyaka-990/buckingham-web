    ...
    <div className="mx-auto max-w-5xl px-6 pb-24 pt-28">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="grid h-16 w-16 place-items-center rounded-full bg-brass-400/15 text-brass-500">
            <User size={28} />
          </div>
          <div>
        ...
        <div className="flex gap-2">
          {user.role === "admin" && (
            <Link href="/admin" className="flex items-center gap-2 rounded-full bg-forest-800 px-5 py-2.5 text-sm font-medium text-white hover:bg-forest-700">
              <LayoutDashboard size={16} /> Admin
            </Link>
          )}
          <form action={async () => { "use server"; await signOut({ redirectTo: "/" }); }}>
            <button className="flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-sm font-medium hover:border-brass-400">
      ...
      <section className="mt-10">
        ...
        <div className="overflow-hidden rounded-3xl border border-border">
          {myOrders.length === 0 && (
            <p className="bg-surface p-6 text-sm text-muted">No orders yet. <a href="/shop" className="text-brass-500 hover:underline">Browse our dogs →</a></p>
          )}
          {myOrders.map((o) => (
...
function StatCard({ icon: Icon, label, value, href }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string; value: string; href?: string }) {
  const inner = (
    <div className="rounded-3xl border border-border bg-surface p-6 transition hover:border-brass-400">
      <Icon className="mb-3 text-brass-500" />
