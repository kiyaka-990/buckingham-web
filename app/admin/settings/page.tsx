    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        <nav className="flex gap-2 lg:flex-col">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} className={cn("flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition", tab === t.id ? "bg-ochre-400/12 text-accent-ink" : "text-muted hover:bg-foreground/5")}>
        ...
        <form ref={formRef} onSubmit={(e) => { e.preventDefault(); save(); }} className="rounded-3xl border border-border bg-surface p-6">
          ...
          <button type="submit" disabled={pending} className="btn-clay mt-6 flex items-center gap-2 rounded-full px-6 py-2.5 text-sm">
...
function Field({ label, defaultValue }: { label: string; defaultValue?: string }) {
  return (
    <div>
      ...
      <input name={nameOf(label)} defaultValue={defaultValue} className="h-11 w-full rounded-xl border border-border bg-background px-4 text-sm outline-none focus:border-ochre-400" />
    </div>
  );
}
...
function Toggle({ label, desc, defaultOn }: { label: string; desc: string; defaultOn?: boolean }) {
  ...
  return (
    <button type="button" onClick={() => setOn(!on)} className="flex w-full items-center justify-between rounded-xl border border-border p-4 text-left transition hover:bg-foreground/5">
      ...
      <span className={cn("relative h-6 w-11 shrink-0 rounded-full transition", on ? "bg-ochre-400" : "bg-muted/30")}>
