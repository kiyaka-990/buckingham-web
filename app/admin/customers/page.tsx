...
export default function AdminCustomers() {
  ...
  for (const o of orders) {
    const c = map.get(o.email) ?? { name: o.customer, email: o.email, location: o.location, orders: 0, spent: 0 };
    c.orders += 1;
        ...
        {customers.map((c) => (
          <div key={c.email} className="rounded-3xl border border-border bg-surface p-5">
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-full bg-gold-400/15 font-display text-lg font-bold text-gold-500">
                {c.name.charAt(0)}
              </div>
            ...
            <div className="mt-4 flex items-center justify-between text-sm">
              <span className="flex items-center gap-1 text-muted"><MapPin size={13} className="text-gold-500" /> {c.location}</span>
            ...
            <div className="mt-3 border-t border-border pt-3">
              ...
              <p className="font-display text-xl font-bold text-gold-500">{formatPrice(c.spent)}</p>
            </div>
          </div>
        ))}
      </div>
