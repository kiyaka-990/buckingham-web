...
export function ShowroomOverlay({ dog, onClose }: { dog: Dog; onClose: () => void }) {
  return (
    ...
    >
      <button onClick={onClose} aria-label="Close" className="absolute right-3 top-3 z-10 grid h-8 w-8 place-items-center rounded-full bg-leaf-950/50 text-white">
        <X size={16} />
      </button>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <Link href={`/dogs/${dog.slug}`} className="btn-leaf flex items-center gap-1 rounded-full px-4 py-2 text-sm">
