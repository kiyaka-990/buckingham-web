...
export function Footer() {
  return (
    <footer className="relative mt-24 border-t border-border bg-clay-950 text-clay-50">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        ...
        <div className="mb-12 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {trustBadges.map((b) => (
            <div key={b.label} className="flex items-center gap-3 rounded-2xl glass p-4">
              <b.icon className="text-ochre-400" />
        ...
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4 lg:grid-cols-5">
          <div className="col-span-2 space-y-4">
            <Link href="/" className="flex items-center gap-3">
              ...
              <span>
                ...
                <span className="text-xs uppercase tracking-[0.3em] text-ochre-400">Limited</span>
              </span>
            </Link>
            <p className="max-w-sm text-sm text-clay-100/80">{site.description}</p>
            <p className="text-xs text-clay-100/60">Reg. No. {site.registration}</p>
            <div className="flex gap-2">
              {[FacebookIcon, InstagramIcon, YoutubeIcon, TiktokIcon].map((Icon, i) => (
                <a key={i} href="#" aria-label="Social link" className="grid h-9 w-9 place-items-center rounded-full glass transition hover:text-ochre-400">
                  <Icon size={16} />
                </a>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            <p className="flex items-start gap-2 text-sm text-clay-100/80">
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-6 text-xs text-clay-100/60 sm:flex-row">
          <p>© {new Date().getFullYear()} {site.name}. All rights reserved.</p>
          <div className="flex gap-4">
            <Link href="/privacy" className="hover:text-ochre-400">Privacy</Link>
            <Link href="/terms" className="hover:text-ochre-400">Terms</Link>
            <Link href="/contact" className="hover:text-ochre-400">Support</Link>
          </div>
        </div>
      </div>
    </footer>
...
function FooterCol({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div className="space-y-3">
      <h4 className="font-display font-semibold text-ochre-400">{title}</h4>
      <ul className="space-y-2">
        {links.map((l) => (
          <li key={l.href}>
            <Link href={l.href} className="text-sm text-clay-100/80 transition hover:text-ochre-400">{l.label}</Link>
          </li>
        ))}
      </ul>
    </div>
