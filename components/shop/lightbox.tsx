    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[95] flex flex-col bg-clay-950/95 backdrop-blur-sm"
        >
          {/* Top bar */}
          ...
          >
            ...
            {scale === 1 && (
              <span className="pointer-events-none absolute bottom-24 left-1/2 flex -translate-x-1/2 items-center gap-1.5 rounded-full glass px-3 py-1.5 text-xs text-white">
                <Maximize2 size={12} className="text-ochre-400" /> Pinch, scroll or double-tap to zoom
              </span>
            )}
          </div>
            ...
            <div className="flex justify-center gap-2 p-4">
              {images.map((src, i) => (
                <button
                  ...
                  className={cn("relative h-14 w-14 overflow-hidden rounded-lg border-2 transition", i === index ? "border-ochre-400" : "border-transparent opacity-50 hover:opacity-100")}
                >
