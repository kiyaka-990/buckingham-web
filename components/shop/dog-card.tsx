...
const statusStyles: Record<Dog["status"], string> = {
  ...
  reserved: "bg-gold-400 text-navy-900",
  sold: "bg-navy-900/80 text-white",
};
    ...
    >
      <Link href={`/dogs/${dog.slug}`} className="relative block aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-2">
          ...
          {dog.bestseller && (
            <span className="rounded-full bg-white/90 px-3 py-1 text-[11px] font-semibold text-navy-900">
              Bestseller
            </span>
          )}
        </div>
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full glass-strong px-2.5 py-1 text-[11px] text-foreground">
          <MapPin size={12} className="text-gold-400" /> {dog.location}
        </div>
      </Link>

      <div className="flex flex-1 flex-col gap-2 p-4">
        <div className="flex items-center justify-between gap-2">
          <Link href={`/breeds/${dog.breedSlug}`} className="text-xs font-medium uppercase tracking-wide text-gold-500 hover:underline">
            {dog.breedName}
          </Link>
        ...
        <Link href={`/dogs/${dog.slug}`}>
          <h3 className="font-display text-lg font-semibold leading-tight group-hover:text-gold-500 transition-colors">
            {dog.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <button
            ...
            className="btn-gold grid h-10 w-10 place-items-center rounded-full disabled:cursor-not-allowed disabled:bg-navy-800 disabled:text-white/50"
          >
            <ShoppingBag size={16} />
          </button>
        </div>
