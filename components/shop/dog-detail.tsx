      ...
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div>
          <div className="flex items-center gap-2 text-sm text-muted">
            ...
            <Link href={`/breeds/${dog.breedSlug}`} className="hover:text-gold-500">{dog.breedName}</Link>
          </div>

          <div className="mt-2 flex items-start justify-between gap-4">
            <h1 className="font-display text-4xl font-bold">{dog.name}</h1>
            <div className="flex gap-2">
              <button onClick={() => toggleWish(dog.id)} aria-label="Wishlist" className="grid h-11 w-11 place-items-center rounded-full border border-border transition hover:border-gold-400">
                <Heart size={18} className={cn(wished && "fill-red-500 text-red-500")} />
              </button>
              <button aria-label="Share" className="grid h-11 w-11 place-items-center rounded-full border border-border transition hover:border-gold-400">
                <Share2 size={18} />
              </button>
            </div>
          </div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3">
            ...
            <span className="flex items-center gap-1 text-sm text-muted"><MapPin size={14} className="text-gold-500" /> {dog.location}, Kenya</span>
          ...
          <div className="mt-6 flex items-end gap-3">
            <span className="font-display text-4xl font-bold text-gradient-gold">{formatPrice(dog.price)}</span>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {dog.traits.map((t) => (
              <span key={t} className="rounded-full bg-gold-400/10 px-3 py-1 text-xs font-medium text-gold-600 dark:text-gold-400">{t}</span>
            ))}
          </div>
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <button
              onClick={addToCart}
              disabled={soldOut}
              className="btn-gold flex h-14 flex-1 items-center justify-center gap-2 rounded-full text-base disabled:cursor-not-allowed disabled:bg-navy-800 disabled:text-white/60"
            >
            ...
            <button
              ...
              disabled={soldOut}
              className="flex h-14 flex-1 items-center justify-center gap-2 rounded-full border border-gold-400 text-base font-semibold text-gold-600 transition hover:bg-gold-400/10 disabled:opacity-40 dark:text-gold-400"
            >
              Reserve Now
            </button>
          </div>
          <a
            ...
            className="mt-3 flex h-12 items-center justify-center gap-2 rounded-full border border-border text-sm font-medium transition hover:border-gold-400"
          >
            Enquire on WhatsApp
          </a>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              <div key={a.t} className="flex items-center gap-2 text-muted">
                <a.icon size={16} className="text-gold-500" /> {a.t}
              </div>
            ))}
          </div>
            ...
            <div className="flex gap-1 border-b border-border">
              {(["overview", "pedigree", "health"] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={cn("relative px-4 py-3 text-sm font-medium capitalize transition", tab === t ? "text-gold-600 dark:text-gold-400" : "text-muted hover:text-foreground")}
                >
                  {t}
                  {tab === t && <motion.span layoutId="tab" className="absolute inset-x-0 -bottom-px h-0.5 bg-gold-400" />}
                </button>
              ))}
            </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {dog.pedigree.champions.map((c) => (
                        <li key={c} className="flex items-center gap-2"><Award size={14} className="text-gold-500" /> {c}</li>
                      ))}
                    </ul>
                  </div>
                </div>
...
function PedigreeCard({ label, name, sub }: { label: string; name: string; sub: string }) {
  return (
    <div className="rounded-2xl border border-border bg-surface p-4">
      <p className="text-[11px] uppercase tracking-wide text-gold-500">{label}</p>
...
function HealthItem({ icon: Icon, label, ok }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string; ok: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3">
      <span className="flex items-center gap-2 text-foreground"><Icon size={16} className="text-gold-500" /> {label}</span>
