    ...
    <div className="mx-auto max-w-7xl px-6 py-10">
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <div className="group relative">
            ...
            <div className="absolute right-4 top-4 z-20 flex gap-2">
              {!spin && (
                <button
                  onClick={() => setLightbox(true)}
                  className="grid h-9 w-9 place-items-center rounded-full glass-strong transition hover:text-accent-ink"
              ...
              {canSpin && (
                <button
                  onClick={() => setSpin((s) => !s)}
                  className="flex items-center gap-1.5 rounded-full glass-strong px-3 py-1.5 text-xs font-semibold transition hover:text-accent-ink"
                  aria-pressed={spin}
                >
                  <Rotate3d size={14} className="text-accent-ink" /> {spin ? "Exit 360°" : "360° View"}
                </button>
              )}
            </div>
          </div>
          ...
          <div className={cn("mt-4 gap-3", dog.images.length > 1 ? "flex" : "hidden")}>
        ...
        <div>
          <div className="flex items-center gap-2 text-sm text-muted">
            <Link href="/shop" className="hover:text-accent-ink">Shop</Link>
            <ChevronRight size={12} />
            <Link href={`/breeds/${dog.breedSlug}`} className="hover:text-accent-ink">{dog.breedName}</Link>
          </div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3">
            ...
            <span className="flex items-center gap-1 text-sm text-muted"><MapPin size={14} className="text-accent-ink" /> {dog.location}, Kenya</span>
...
function HealthItem({ icon: Icon, label, ok }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string; ok: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3">
      <span className="flex items-center gap-2 text-foreground"><Icon size={16} className="text-accent-ink" /> {label}</span>
