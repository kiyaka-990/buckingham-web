...
/** Auto-generated "Special Offers" strip from catalog dogs on sale (compareAt set). */
export function SpecialOffers() {
  const deals = dogs.filter((d) => d.compareAt && d.status !== "sold");
  if (deals.length === 0) return null;

  return (
    <section className="mx-auto max-w-7xl px-6 pt-10">
      <div className="relative overflow-hidden rounded-[2rem] border border-gold-400/30 bg-royal p-6 sm:p-8">
        <div className="aurora pointer-events-none absolute inset-0 opacity-40" />
        <div className="relative">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3 text-white">
              <span className="grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br from-red-500 to-gold-400 text-navy-900">
                <Flame size={22} />
              </span>
              <div>
            ...
            <span className="flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white">
              <Tag size={13} className="text-gold-400" /> {deals.length} on sale
            </span>
          </div>
            ...
            {deals.map((d) => {
              ...
              return (
                <Link
                  key={d.id}
                  href={`/dogs/${d.slug}`}
                  className="group relative w-56 shrink-0 snap-start overflow-hidden rounded-2xl border border-white/10 bg-navy-900/40 backdrop-blur"
                >
                  ...
                  <div className="p-3 text-white">
                    <p className="text-[11px] uppercase tracking-wide text-gold-400">{d.breedName}</p>
                    <p className="font-display font-semibold leading-tight">{d.name}</p>
                    <div className="mt-1 flex items-baseline gap-2">
                      <span className="font-display text-lg font-bold text-gold-400">{formatPrice(d.price)}</span>
                      <span className="text-xs text-white/50 line-through">{formatPrice(d.compareAt as number)}</span>
                    </div>
                    <span className="mt-1 inline-flex items-center gap-1 text-xs font-medium text-white/80 transition-all group-hover:gap-2 group-hover:text-gold-400">
