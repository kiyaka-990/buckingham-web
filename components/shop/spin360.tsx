...
export function Spin360({ images, className }: { images: string[]; className?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* 360 badge */}
      <span className="absolute left-4 top-4 flex items-center gap-1.5 rounded-full bg-forest-950/70 px-3 py-1 text-xs font-semibold text-white backdrop-blur">
        <Rotate3d size={14} className="text-brass-400" /> 360°
      </span>
      ...
      <span className={cn("absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-1.5 rounded-full glass-strong px-3 py-1.5 text-xs text-white transition-opacity", dragging ? "opacity-0" : "opacity-100")}>
        <MousePointer2 size={13} className="text-brass-400" /> Drag to rotate
      </span>

      {/* Frame dots */}
      <div className="absolute bottom-4 right-4 flex gap-1">
        {images.map((_, i) => (
          <span key={i} className={cn("h-1.5 rounded-full transition-all", i === frame ? "w-4 bg-brass-400" : "w-1.5 bg-white/40")} />
        ))}
      </div>
    </div>
  );
