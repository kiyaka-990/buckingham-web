  ...
  if (status === "sent") {
    return (
      <div className="flex flex-col items-center justify-center rounded-3xl border border-border bg-surface p-12 text-center">
        ...
        <button onClick={() => setStatus("idle")} className="mt-6 text-sm text-gold-500 hover:underline">Send another message</button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-4 rounded-3xl border border-border bg-surface p-6 sm:p-8">
      ...
      <div>
        ...
        <select name="interest" className="h-12 w-full rounded-xl border border-border bg-background px-4 outline-none focus:border-gold-400">
      ...
      <div>
        ...
        <textarea name="message" rows={4} required placeholder="Tell us what you're looking for…" className="w-full rounded-xl border border-border bg-background px-4 py-3 outline-none focus:border-gold-400" />
      </div>
      <button disabled={status === "sending"} className="btn-gold flex h-13 w-full items-center justify-center gap-2 rounded-full py-3.5 text-base">
...
function Field({ label, name, type = "text", required, placeholder }: { label: string; name: string; type?: string; required?: boolean; placeholder?: string }) {
  return (
    <div>
      ...
      <input name={name} type={type} required={required} placeholder={placeholder} className="h-12 w-full rounded-xl border border-border bg-background px-4 outline-none focus:border-gold-400" />
    </div>
  );
}
