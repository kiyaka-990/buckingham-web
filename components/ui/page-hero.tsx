    ...
    <section className="relative flex min-h-[42vh] items-end overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 pb-12 pt-28 text-white">
        <nav className="mb-4 flex items-center gap-1 text-xs text-white/70">
          <Link href="/" className="hover:text-brass-400">Home</Link>
          {crumbs.map((c) => (
            <span key={c.label} className="flex items-center gap-1">
              <ChevronRight size={12} />
              {c.href ? <Link href={c.href} className="hover:text-brass-400">{c.label}</Link> : <span className="text-brass-400">{c.label}</span>}
            </span>
          ))}
        </nav>
        {eyebrow && <p className="mb-2 text-xs font-semibold uppercase tracking-[0.25em] text-brass-400">{eyebrow}</p>}
