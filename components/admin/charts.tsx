...
/* ---------------- Sparkline ---------------- */
export function Sparkline({ data, color = "var(--color-gold-400)", width = 120, height = 36 }: { data: number[]; color?: string; width?: number; height?: number }) {
...
export function AreaChart({ data, labels, height = 240 }: { data: number[]; labels: string[]; height?: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full">
      ...
      <polyline points={line} fill="none" stroke="var(--color-gold-400)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      {data.map((d, i) => (
        <circle key={i} cx={pad + i * step} cy={y(d)} r="3.5" fill="var(--surface)" stroke="var(--color-gold-400)" strokeWidth="2" />
      ))}
      {labels.map((l, i) => (
