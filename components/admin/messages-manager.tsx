        ...
        <div className="rounded-3xl border border-border bg-surface">
          ...
          <ul className="max-h-[60vh] overflow-y-auto">
            {list.map((m) => {
              ...
              return (
                <li key={m.id}>
                  <button onClick={() => open(m)} className={cn("flex w-full gap-3 border-b border-border p-4 text-left transition hover:bg-foreground/[0.03]", selected?.id === m.id && "bg-ochre-400/5")}>
                    ...
                    {m.unread && <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-ochre-400" />}
                  </button>
                </li>
              );
            })}
            ...
            <>
              <div className="flex items-start justify-between gap-3 border-b border-border pb-4">
                <div className="flex items-center gap-3">
                  <span className="grid h-12 w-12 place-items-center rounded-full bg-ochre-400/15 font-display text-lg font-bold text-accent-ink">{selected.name.charAt(0)}</span>
              ...
              <div className="mt-6 rounded-2xl border border-border bg-background p-3">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <button onClick={sendReply} disabled={pending} className="btn-clay flex items-center gap-2 rounded-full px-5 py-2 text-sm">
